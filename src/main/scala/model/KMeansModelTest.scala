...
import process.{AttributeSpecification, DataClean}
...
object KMeansModelTest {
  ...
  def analyse_center(model:KMeansModel,data:DataFrame)={
    ...
    val tmp = pre_data.rdd.map(x=>(x(3).toString.toDouble,x(1).toString.split(","))).flatMap{x=>tmp1(x._1,x._2)}.map(x=>((x._1._1,x._1._2,x._2),1))
  ...
  def main(args: Array[String]): Unit = {
    ...
    val data_exchange=dataExchange(data_specification,200)
